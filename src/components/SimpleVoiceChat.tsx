...
const SimpleVoiceChat: React.FC<SimpleVoiceChatProps> = ({ 
  disabled = false,
  className = "",
  onSendMessage
}) => {
  ...
  useEffect(() => {
    ...
    const checkSupport = () => {
      ...
      console.log('Simple voice chat support check:', { hasRecognition, hasSynthesis });
  ...
  const initializeSpeech = () => {
    try {
      ...
      recognitionRef.current.onstart = () => {
        console.log('Simple voice chat recognition started');
        setIsListening(true);
        setError(null);
      };
      ...
      recognitionRef.current.onresult = async (event: any) => {
        ...
        console.log('Simple voice chat recognized:', transcript);
        ...
        if (transcript.trim() && onSendMessage) {
          ...
          // Чекаємо трохи і отримуємо відповідь
          setTimeout(async () => {
            await getAIResponse(transcript.trim());
          }, 1000);
        }
      };

      recognitionRef.current.onerror = (event: any) => {
        console.error('Simple voice chat recognition error:', event.error);
      ...
      recognitionRef.current.onend = () => {
        console.log('Simple voice chat recognition ended');
        setIsListening(false);
      };
      ...
      console.error('Simple voice chat initialization error:', error);
  ...
  const getAIResponse = async (userMessage: string) => {
    try {
      ...
      if (response.ok) {
        ...
        speakResponse('Вибачте, не вдалося отримати відповідь');
      }
    } catch (error) {
      console.error('Error getting AI response:', error);
      speakResponse('Вибачте, сталася помилка');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const startListening = () => {
    ...
    try {
      ...
      console.error('Error starting simple voice chat:', error);
